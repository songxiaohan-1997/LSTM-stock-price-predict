...
def data2change(data):
    ...
    change = change.fillna(0.).values.tolist()#all set as float
    change = [c[0] for c in change]
    return change



def load_text_csv(filename = 'Combined_News_SP.csv', date_split = date(2014,12,31)):
	'''
...
def load_ts_csv(filename = 'SP.csv', date_split = date(2014,12,31)):	
	''' 
...
def transform_text_into_vectors(train_text, test_text, embedding_size = 100, model_path = 'word2vec10.model'):#embedding: potential factor of scentence
	'''
	...
	model = Word2Vec(data_for_w2v, size=embedding_size, window=5, min_count=1)
  ...
  #  for i in range(0, len(data_chng_train)-window-forecast, step): 
   #     try:
    #        x_i = data_chng_train[i:i+window]
     #       y_i = np.std(data_chng_train[i:i+window+forecast][3])#volatility of stock price in each window close
    #
     #       text_average = train_text_vectors[i:i+window]
      #      last_close = x_i[-1]#last
    #
     #       y_i2 = None
      #      if data_chng_train[i+window+forecast][3] > 0.:#if stock price increase on the day after window then 1
       #         y_i2 = 1.
        #    else:
         #       y_i2 = 0.
    #
     #   except Exception as e:
      #      print ('KEK')
       #     print (e)
        #break
        #x_i=x_i.tolist()
        #X_train.append(x_i)#每个window里的所有数据
      #  X_train_text.append(text_average)#step number*window*（100）每个window里的天的新闻向量
       # Y_train.append(y_i)#step number*window*1 变化率向量
        #Y_train2.append(y_i2)#step number*1每个window之后股价上还是下
    #print(y_i)
    #print(X_train)
    #print(X_train_text)
    #print(Y_train2)
